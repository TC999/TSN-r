 ...
 */
    ...
    private final Request buildRedirectRequest(Response response, String str) {
        ...
        if (f0.g(resolve.scheme(), response.request().url().scheme()) || this.client.followSslRedirects()) {
            Request.Builder newBuilder = response.request().newBuilder();
            if (HttpMethod.permitsRequestBody(str)) {
                int code = response.code();
                HttpMethod httpMethod = HttpMethod.INSTANCE;
                boolean z3 = httpMethod.redirectsWithBody(str) || code == 308 || code == 307;
    ...
    private final boolean recover(IOException iOException, RealCall realCall, Request request, boolean z3) {
        if (this.client.retryOnConnectionFailure()) {
            return !(z3 && requestIsOneShot(iOException, request)) && isRecoverable(iOException, z3) && realCall.retryAfterFailure();
        }
        return false;
    }
                ...
                if (!call$okhttp.isCanceled()) {
                    try {
                        ...
                        if (recover(e4, call$okhttp, request$okhttp, !(e4 instanceof ConnectionShutdownException))) {
                            F = kotlin.collections.f0.p4(F, e4);
                            call$okhttp.exitNetworkInterceptorExchange$okhttp(true);
                            z3 = false;
                        } else {
                            throw Util.withSuppressed(e4, F);
                        }
                    } catch (RouteException e5) {
                        if (recover(e5.getLastConnectException(), call$okhttp, request$okhttp, false)) {
                            F = kotlin.collections.f0.p4(F, e5.getFirstConnectException());
                            call$okhttp.exitNetworkInterceptorExchange$okhttp(true);
                            z3 = false;
                        } else {
                            throw Util.withSuppressed(e5.getFirstConnectException(), F);
                        }
                    }
