...
public class CommonNetImpl implements UMLogDataProtocol {
    ...
    private void saveFile(Object obj, int i4) {
        if (obj != null && (obj instanceof JSONObject)) {
            JSONObject jSONObject = (JSONObject) obj;
            switch (i4) {
                case 24577:
                    UMRTLog.e("MobclickRT", "--->>> Share: DAU_EVENT");
                    DBManager.get(this.mConetxt).insertDau(jSONObject);
                    break;
                case 24578:
                    UMRTLog.e("MobclickRT", "--->>> Share: SHARE_EVENT");
                    DBManager.get(this.mConetxt).insertS_E(jSONObject);
                    break;
                case 24579:
                    UMRTLog.e("MobclickRT", "--->>> Share: AUTH_EVENT");
                    DBManager.get(this.mConetxt).insertAuth(jSONObject);
                    break;
                case 24580:
                    UMRTLog.e("MobclickRT", "--->>> Share: GET_EVENT");
                    DBManager.get(this.mConetxt).insertUserInfo(jSONObject);
                    break;
                case 24581:
                case 24583:
                    UMRTLog.e("MobclickRT", "--->>> Share: SAVE_STATS_EVENT");
                    DBManager.get(this.mConetxt).insertStats(jSONObject);
                    break;
                case 24582:
                default:
                    UMRTLog.e("MobclickRT", "--->>> Share: default case.");
                    DBManager.get(this.mConetxt).insertStats(jSONObject);
                    break;
            }
    ...
    public JSONObject readData(Context context, int i4) {
        JSONArray jSONArray;
        ...
        boolean z3 = 1048576.0d <= checkFile + 24576.0d;
        JSONObject jSONObject = new JSONObject();
        try {
            new JSONObject();
            JSONObject jSONObject2 = new JSONObject();
            JSONArray select = DBManager.get(context).select("s_e", this.shareList, 1047552.0d, z3);
            double length = select.toString().getBytes().length;
            Double.isNaN(length);
            double d4 = length + 1024.0d;
            JSONArray select2 = DBManager.get(context).select("auth", this.authList, 1048576.0d - d4, z3);
            double length2 = select2.toString().getBytes().length;
            Double.isNaN(length2);
            double d5 = d4 + length2;
            JSONArray select3 = DBManager.get(context).select("userinfo", this.infoList, 1048576.0d - d5, z3);
            double length3 = select3.toString().getBytes().length;
            Double.isNaN(length3);
            double d6 = d5 + length3;
            JSONArray jSONArray5 = select3;
            JSONArray select4 = DBManager.get(context).select("dau", this.dauList, 1048576.0d - d6, z3);
            double length4 = select4.toString().getBytes().length;
            Double.isNaN(length4);
            JSONArray select5 = DBManager.get(context).select("stats", this.statsList, 1048576.0d - (d6 + length4), z3);
    ...
    public void workEvent(Object obj, int i4) {
        switch (i4) {
            case 24592:
                UMRTLog.e("MobclickRT", "--->>> Share: REGIST_TO_WORK_QUEUE");
                return;
            case 24593:
                UMRTLog.e("MobclickRT", "--->>> Share: BUILD_ENVELOPE");
                JSONObject constructHeader = constructHeader();
                if (constructHeader != null) {
                    JSONObject readData = readData(this.mConetxt, i4);
                    if (readData != null) {
                        ...
                        if (buildEnvelopeWithExtHeader != null) {
                            if (buildEnvelopeWithExtHeader.has("exception")) {
                                UMRTLog.e("MobclickRT", "--->>> Share: \u6784\u5efa\u4fe1\u5c01\u5931\u8d25!");
                                if (readData.optInt("exception") != 101) {
                                    deleteData(this.mConetxt);
                                }
                            } else {
                                UMRTLog.e("MobclickRT", "--->>> Share: \u6784\u5efa\u4fe1\u5c01\u6210\u529f\uff0c\u5220\u9664\u672c\u5730\u6570\u636e!");
                                deleteData(this.mConetxt);
                            }
                        }
                    } else {
                ...
                return;
            case 24594:
                UMRTLog.e("MobclickRT", "--->>> Share: AZX_TRIGGER");
                try {
                    h.a();
                    return;
                } catch (Throwable th) {
                    UMRTLog.e("MobclickRT", "--->>> Share: AZX_TASK invoke exception!");
                    th.printStackTrace();
                    return;
                }
            default:
