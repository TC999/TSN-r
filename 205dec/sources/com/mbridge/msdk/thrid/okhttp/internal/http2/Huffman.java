...
class Huffman {
    ...
    private void addCode(int i4, int i5, byte b4) {
        Node node = new Node(i4, b4);
    ...
    public byte[] decode(byte[] bArr) {
        ...
        for (byte b4 : bArr) {
            ...
            i5 += 8;
            while (i5 >= 8) {
                node = node.children[(i4 >>> (i5 - 8)) & 255];
                if (node.children == null) {
                    byteArrayOutputStream.write(node.symbol);
                    i5 -= node.terminalBits;
                    node = this.root;
                } else {
                    i5 -= 8;
                }
        ...
        while (i5 > 0) {
            Node node2 = node.children[(i4 << (8 - i5)) & 255];
            if (node2.children != null || node2.terminalBits > i5) {
                break;
            }
            byteArrayOutputStream.write(node2.symbol);
            i5 -= node2.terminalBits;
            node = this.root;
        }
    ...
    public void encode(ByteString byteString, BufferedSink bufferedSink) throws IOException {
        ...
        long j4 = 0;
        byte b4 = 0;
        while (i4 < byteString.size()) {
            int i5 = byteString.getByte(i4) & 255;
            int i6 = CODES[i5];
            byte b5 = CODE_LENGTHS[i5];
            j4 = (j4 << b5) | i6;
            int i7 = b4 + b5;
            while (i7 >= 8) {
                i7 = (i7 == 1 ? 1 : 0) - 8;
                bufferedSink.writeByte((int) (j4 >> i7));
            }
            i4++;
            b4 = i7;
        }
        if (b4 > 0) {
            bufferedSink.writeByte((int) ((255 >>> b4) | (j4 << (8 - b4))));
        }
    }
    ...
    public int encodedLength(ByteString byteString) {
        ...
        for (int i4 = 0; i4 < byteString.size(); i4++) {
            j4 += CODE_LENGTHS[byteString.getByte(i4) & 255];
        }
        return (int) ((j4 + 7) >> 3);
    }
