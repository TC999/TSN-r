...
public final class Excluder implements TypeAdapterFactory, Cloneable {
    ...
    private boolean excludeClassInStrategy(Class<?> cls, boolean z3) {
        for (ExclusionStrategy exclusionStrategy : z3 ? this.serializationStrategies : this.deserializationStrategies) {
    ...
    public <T> TypeAdapter<T> create(final Gson gson, final TypeToken<T> typeToken) {
        ...
        final boolean z3 = excludeClassChecks || excludeClassInStrategy(rawType, true);
        final boolean z4 = excludeClassChecks || excludeClassInStrategy(rawType, false);
        if (z3 || z4) {
    ...
    public boolean excludeClass(Class<?> cls, boolean z3) {
        return excludeClassChecks(cls) || excludeClassInStrategy(cls, z3);
    }
        ...
        if ((this.version == -1.0d || isValidVersion((Since) field.getAnnotation(Since.class), (Until) field.getAnnotation(Until.class))) && !field.isSynthetic()) {
            if (!this.requireExpose || ((expose = (Expose) field.getAnnotation(Expose.class)) != null && (!z3 ? !expose.deserialize() : !expose.serialize()))) {
                if ((this.serializeInnerClasses || !isInnerClass(field.getType())) && !isAnonymousOrLocal(field.getType())) {
                    List<ExclusionStrategy> list = z3 ? this.serializationStrategies : this.deserializationStrategies;
                    if (list.isEmpty()) {
                        return false;
                    }
    ...
    public Excluder withModifiers(int... iArr) {
        ...
        for (int i4 : iArr) {
            m86clone.modifiers = i4 | m86clone.modifiers;
        }
        return m86clone;
    }
