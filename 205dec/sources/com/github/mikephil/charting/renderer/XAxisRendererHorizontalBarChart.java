...
public class XAxisRendererHorizontalBarChart extends XAxisRenderer {
    ...
    protected void computeSize() {
        ...
        FSize sizeOfRotatedRectangleByDegrees = Utils.getSizeOfRotatedRectangleByDegrees(calcTextSize.width, f4, this.mXAxis.getLabelRotationAngle());
        this.mXAxis.mLabelWidth = Math.round((int) (calcTextSize.width + (this.mXAxis.getXOffset() * 3.5f)));
        this.mXAxis.mLabelHeight = Math.round(f4);
    ...
    protected void drawGridLine(Canvas canvas, float f4, float f5, Path path) {
        path.moveTo(this.mViewPortHandler.contentRight(), f5);
        path.lineTo(this.mViewPortHandler.contentLeft(), f5);
    ...
    protected void drawLabels(Canvas canvas, float f4, MPPointF mPPointF) {
        ...
        for (int i5 = 0; i5 < i4; i5 += 2) {
            if (isCenterAxisLabelsEnabled) {
                fArr[i5 + 1] = this.mXAxis.mCenteredEntries[i5 / 2];
            } else {
                fArr[i5 + 1] = this.mXAxis.mEntries[i5 / 2];
            }
        }
        ...
        for (int i6 = 0; i6 < i4; i6 += 2) {
            ...
            if (this.mViewPortHandler.isInBoundsY(f5)) {
                ...
                XAxis xAxis = this.mXAxis;
                drawLabel(canvas, valueFormatter.getAxisLabel(xAxis.mEntries[i6 / 2], xAxis), f4, f5, mPPointF, labelRotationAngle);
            }
        }
    }
