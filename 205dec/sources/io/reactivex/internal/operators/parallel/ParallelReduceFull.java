 ...
 */
    ...
    public static final class ParallelReduceFullMainSubscriber<T> extends DeferredScalarSubscription<T> {
        ...
        ParallelReduceFullMainSubscriber(Subscriber<? super T> subscriber, int i4, BiFunction<T, T, T> biFunction) {
            super(subscriber);
        ...
        void innerComplete(T t3) {
            if (t3 != null) {
                while (true) {
                    SlotPair<T> addValue = addValue(t3);
                    if (addValue == null) {
                        break;
                    }
                    try {
                        t3 = (T) ObjectHelper.requireNonNull(this.reducer.apply(addValue.first, addValue.second), "The reducer returned a null value");
                    } catch (Throwable th) {
                        Exceptions.throwIfFatal(th);
                        innerError(th);
                        return;
