 ...
 */
    ...
    private static void closeSelector(String str, Selector selector) {
        try {
            ...
            internalLogger.warn("Failed to close a " + str + " selector.", (Throwable) e4);
        }
    }
    ...
    public Set<InetSocketAddress> allRemoteAddresses() {
        try {
            Set<SocketAddress> remoteAddresses = this.ch.getRemoteAddresses();
    ...
    public OioSctpChannel(Channel channel, com.sun.nio.sctp.SctpChannel sctpChannel) {
        ...
        try {
            try {
                ...
                throw new ChannelException("failed to initialize a sctp channel", e4);
            }
        } catch (Throwable th) {
            try {
                sctpChannel.close();
            } catch (IOException e5) {
                logger.warn("Failed to close a sctp channel.", (Throwable) e5);
            }
            throw th;
        }
    }
