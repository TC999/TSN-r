 ...
 */
            ...
            try {
                ...
                if (HttpHeaderNames.CONTENT_DISPOSITION.contentEqualsIgnoreCase(splitMultipartHeader[0])) {
                    if (this.currentStatus == HttpPostRequestDecoder.MultiPartStatus.DISPOSITION) {
                        ...
                        z3 = HttpHeaderValues.ATTACHMENT.contentEqualsIgnoreCase(splitMultipartHeader[1]) || HttpHeaderValues.FILE.contentEqualsIgnoreCase(splitMultipartHeader[1]);
                    }
                    if (z3) {
                        for (int i4 = 2; i4 < splitMultipartHeader.length; i4++) {
                            String[] split = splitMultipartHeader[i4].split("=", 2);
                            try {
                    ...
                    if (asciiString.contentEqualsIgnoreCase(splitMultipartHeader[0])) {
                        ...
                        if (asciiString2.contentEqualsIgnoreCase(splitMultipartHeader[0])) {
                            ...
                            for (int i5 = 1; i5 < splitMultipartHeader.length; i5++) {
                                String lowerCase = splitMultipartHeader[i5].toLowerCase();
                                AsciiString asciiString3 = HttpHeaderValues.CHARSET;
                                if (lowerCase.startsWith(asciiString3.toString())) {
                                    try {
                                        this.currentFieldAttributes.put(asciiString3, this.factory.createAttribute(this.request, asciiString3.toString(), cleanString(StringUtil.substringAfter(splitMultipartHeader[i5], '='))));
                                    ...
                                    try {
                                        Attribute createAttribute2 = this.factory.createAttribute(this.request, cleanString(splitMultipartHeader[0]), splitMultipartHeader[i5]);
    ...
    */
    ...
    private String readDelimiterStandard(String str) {
        ...
        try {
            ...
            int i4 = 0;
            int length = str.length();
            while (this.undecodedChunk.isReadable() && i4 < length) {
    ...
    */
    ...
    public void destroy() {
        ...
        for (int i4 = this.bodyListHttpDataRank; i4 < this.bodyListHttpData.size(); i4++) {
