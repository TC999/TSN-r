...
public abstract class HttpObjectEncoder<H extends HttpMessage> extends MessageToMessageEncoder<Object> {
    ...
    private void encodeChunkedContent(ChannelHandlerContext channelHandlerContext, Object obj, long j4, List<Object> list) {
        if (j4 > 0) {
            byte[] bytes = Long.toHexString(j4).getBytes(CharsetUtil.US_ASCII);
    ...
    public void encode(ChannelHandlerContext channelHandlerContext, Object obj, List<Object> list) throws Exception {
        ...
        boolean z3 = obj instanceof ByteBuf;
        if (z3 && !((ByteBuf) obj).isReadable()) {
            list.add(Unpooled.EMPTY_BUFFER);
            return;
        }
        boolean z4 = obj instanceof HttpContent;
        if (!z4 && !z3 && !(obj instanceof FileRegion)) {
            if (byteBuf != null) {
                list.add(byteBuf);
            }
        } else if (this.state != 0) {
            ...
            if (contentLength > 0) {
                if (byteBuf != null && byteBuf.writableBytes() >= contentLength && z4) {
