 ...
 */
    ...
    public JdkZlibEncoder(int i4) {
        this(ZlibWrapper.ZLIB, i4);
    }

    /* JADX INFO: Access modifiers changed from: protected */
    @Override // io.netty.handler.codec.MessageToByteEncoder
    public final ByteBuf allocateBuffer(ChannelHandlerContext channelHandlerContext, ByteBuf byteBuf, boolean z3) throws Exception {
        double readableBytes = byteBuf.readableBytes();
        Double.isNaN(readableBytes);
        int ceil = ((int) Math.ceil(readableBytes * 1.001d)) + 12;
        if (this.writeHeader) {
            int i4 = AnonymousClass4.$SwitchMap$io$netty$handler$codec$compression$ZlibWrapper[this.wrapper.ordinal()];
            if (i4 == 1) {
    ...
    public void encode(ChannelHandlerContext channelHandlerContext, ByteBuf byteBuf, ByteBuf byteBuf2) throws Exception {
        ...
        if (this.wrapper == ZlibWrapper.GZIP) {
            this.crc.update(bArr, i4, readableBytes);
        }
        this.deflater.setInput(bArr, i4, readableBytes);
    ...
    public JdkZlibEncoder(ZlibWrapper zlibWrapper, int i4) {
        this.crc = new CRC32();
        ...
        if (i4 < 0 || i4 > 9) {
            ...
            if (zlibWrapper != zlibWrapper2) {
                ...
                this.deflater = new Deflater(i4, zlibWrapper != ZlibWrapper.ZLIB);
                return;
            }
    ...
    public JdkZlibEncoder(int i4, byte[] bArr) {
        this.crc = new CRC32();
        this.writeHeader = true;
        if (i4 < 0 || i4 > 9) {
            throw new IllegalArgumentException("compressionLevel: " + i4 + " (expected: 0-9)");
