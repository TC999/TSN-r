 ...
 */
    ...
    private void closeBlock(ByteBuf byteBuf) {
        ...
        int i4 = this.streamCRC;
        this.streamCRC = crc ^ ((i4 >>> 31) | (i4 << 1));
    }
    ...
    public ChannelFuture finishEncode(ChannelHandlerContext channelHandlerContext, ChannelPromise channelPromise) {
        ...
        try {
            ...
            bzip2BitWriter.writeInt(buffer, i4);
    ...
    public Bzip2Encoder(int i4) {
        this.currentState = State.INIT;
        this.writer = new Bzip2BitWriter();
        if (i4 >= 1 && i4 <= 9) {
            this.streamBlockSize = i4 * 100000;
            return;
        }
        throw new IllegalArgumentException("blockSizeMultiplier: " + i4 + " (expected: 1-9)");
    }
