...
public class SnappyFrameEncoder extends MessageToByteEncoder<ByteBuf> {
    ...
    private static void setChunkLength(ByteBuf byteBuf, int i4) {
        int writerIndex = (byteBuf.writerIndex() - i4) - 3;
        if ((writerIndex >>> 24) == 0) {
            byteBuf.setMediumLE(i4, writerIndex);
            return;
        }
        throw new CompressionException("compressed data too large: " + writerIndex);
    }

    private static void writeChunkLength(ByteBuf byteBuf, int i4) {
        byteBuf.writeMediumLE(i4);
    }

    private static void writeUnencodedChunk(ByteBuf byteBuf, ByteBuf byteBuf2, int i4) {
        byteBuf2.writeByte(1);
        writeChunkLength(byteBuf2, i4 + 4);
