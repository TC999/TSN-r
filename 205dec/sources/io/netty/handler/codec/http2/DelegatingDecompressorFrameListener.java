    ...
    public final class ConsumedBytesConverter implements Http2LocalFlowController {
        ...
        public boolean consumeBytes(Http2Stream http2Stream, int i4) throws Http2Exception {
            ...
            if (decompressor != null) {
                i4 = decompressor.consumeBytes(http2Stream.id(), i4);
            }
            try {
                return this.flowController.consumeBytes(http2Stream, i4);
 ...
 */
    ...
    public static final class Http2Decompressor {
        ...
        int consumeBytes(int i4, int i5) throws Http2Exception {
            if (i5 >= 0) {
                int i6 = this.decompressed;
                if (i6 - i5 >= 0) {
                    double d4 = i5;
                    double d5 = i6;
                    Double.isNaN(d4);
                    Double.isNaN(d5);
                    double d6 = d4 / d5;
                    int i7 = this.compressed;
                    double d7 = i7;
                    Double.isNaN(d7);
                    int min = Math.min(i7, (int) Math.ceil(d7 * d6));
                    int i8 = this.compressed;
                    if (i8 - min >= 0) {
                        this.decompressed -= i5;
                        this.compressed = i8 - min;
                        return min;
                    }
                    throw Http2Exception.streamError(i4, Http2Error.INTERNAL_ERROR, "overflow when converting decompressed bytes to compressed bytes for stream %d.decompressedBytes: %d decompressed: %d compressed: %d consumedCompressed: %d", Integer.valueOf(i4), Integer.valueOf(i5), Integer.valueOf(this.decompressed), Integer.valueOf(this.compressed), Integer.valueOf(min));
                }
                throw Http2Exception.streamError(i4, Http2Error.INTERNAL_ERROR, "Attempting to return too many bytes for stream %d. decompressed: %d decompressedBytes: %d", Integer.valueOf(i4), Integer.valueOf(this.decompressed), Integer.valueOf(i5));
            }
            throw new IllegalArgumentException("decompressedBytes must not be negative: " + i5);
        }
        ...
        void incrementCompressedBytes(int i4) {
            this.compressed += i4;
        }

        void incrementDecompressedBytes(int i4) {
            this.decompressed += i4;
        }
    }
    ...
    private void initDecompressor(ChannelHandlerContext channelHandlerContext, int i4, Http2Headers http2Headers, boolean z3) throws Http2Exception {
        Http2Stream stream = this.connection.stream(i4);
        if (stream == null) {
            return;
        }
        Http2Decompressor decompressor = decompressor(stream);
        if (decompressor == null && !z3) {
    ...
    public int onDataRead(ChannelHandlerContext channelHandlerContext, int i4, ByteBuf byteBuf, int i5, boolean z3) throws Http2Exception {
        ...
        try {
            ...
            if (nextReadableBuf == null && z3 && decompressor2.finish()) {
                nextReadableBuf = nextReadableBuf(decompressor2);
            }
            if (nextReadableBuf == null) {
                if (z3) {
                    this.listener.onDataRead(channelHandlerContext, i4, Unpooled.EMPTY_BUFFER, i5, true);
                }
    ...
    public void onHeadersRead(ChannelHandlerContext channelHandlerContext, int i4, Http2Headers http2Headers, int i5, boolean z3) throws Http2Exception {
        initDecompressor(channelHandlerContext, i4, http2Headers, z3);
        this.listener.onHeadersRead(channelHandlerContext, i4, http2Headers, i5, z3);
    }

    public DelegatingDecompressorFrameListener(Http2Connection http2Connection, Http2FrameListener http2FrameListener, boolean z3) {
    ...
    public void onHeadersRead(ChannelHandlerContext channelHandlerContext, int i4, Http2Headers http2Headers, int i5, short s3, boolean z3, int i6, boolean z4) throws Http2Exception {
        initDecompressor(channelHandlerContext, i4, http2Headers, z4);
