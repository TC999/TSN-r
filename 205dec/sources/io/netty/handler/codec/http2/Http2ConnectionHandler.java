 ...
 */
    ...
    class AnonymousClass5 implements ChannelFutureListener {
        ...
        AnonymousClass5(ChannelHandlerContext channelHandlerContext, int i4, long j4, ByteBuf byteBuf) {
            this.val$ctx = channelHandlerContext;
            this.val$lastStreamId = i4;
            this.val$errorCode = j4;
 ...
 */
    ...
    public static final class ClosingChannelFutureListener implements ChannelFutureListener {
        ...
        ClosingChannelFutureListener(final ChannelHandlerContext channelHandlerContext, final ChannelPromise channelPromise, long j4, TimeUnit timeUnit) {
            this.ctx = channelHandlerContext;
            this.promise = channelPromise;
 ...
 */
    ...
    public void checkCloseConnection(ChannelFuture channelFuture) {
        ...
        try {
            ...
            throw new IllegalStateException("Close listener threw an unexpected exception", e4);
        }
    }
    ...
    /* JADX INFO: Access modifiers changed from: private */
    public static void processGoAwayWriteResult(ChannelHandlerContext channelHandlerContext, int i4, long j4, ByteBuf byteBuf, ChannelFuture channelFuture) {
        try {
            if (channelFuture.isSuccess()) {
                if (j4 != Http2Error.NO_ERROR.code()) {
                    InternalLogger internalLogger = logger;
                    if (internalLogger.isDebugEnabled()) {
                        internalLogger.debug("{} Sent GOAWAY: lastStreamId '{}', errorCode '{}', debugData '{}'. Forcing shutdown of the connection.", channelHandlerContext.channel(), Integer.valueOf(i4), Long.valueOf(j4), byteBuf.toString(CharsetUtil.UTF_8), channelFuture.cause());
                    }
                ...
                if (internalLogger2.isDebugEnabled()) {
                    internalLogger2.debug("{} Sending GOAWAY failed: lastStreamId '{}', errorCode '{}', debugData '{}'. Forcing shutdown of the connection.", channelHandlerContext.channel(), Integer.valueOf(i4), Long.valueOf(j4), byteBuf.toString(CharsetUtil.UTF_8), channelFuture.cause());
                }
                channelHandlerContext.close();
            }
        } finally {
    ...
    public void closeStreamLocal(Http2Stream http2Stream, ChannelFuture channelFuture) {
        int i4 = AnonymousClass4.$SwitchMap$io$netty$handler$codec$http2$Http2Stream$State[http2Stream.state().ordinal()];
        if (i4 != 1 && i4 != 2) {
            closeStream(http2Stream, channelFuture);
        } else {
    ...
    @Override // io.netty.handler.codec.http2.Http2LifecycleManager
    public ChannelFuture goAway(final ChannelHandlerContext channelHandlerContext, final int i4, final long j4, final ByteBuf byteBuf, ChannelPromise channelPromise) {
        try {
            Http2Connection connection = connection();
            if (connection.goAwaySent() && i4 > connection.remote().lastStreamKnownByPeer()) {
                throw Http2Exception.connectionError(Http2Error.PROTOCOL_ERROR, "Last stream identifier must not increase between sending multiple GOAWAY frames (was '%d', is '%d').", Integer.valueOf(connection.remote().lastStreamKnownByPeer()), Integer.valueOf(i4));
            }
            connection.goAwaySent(i4, j4, byteBuf);
            byteBuf.retain();
            ChannelFuture writeGoAway = frameWriter().writeGoAway(channelHandlerContext, i4, j4, byteBuf, channelPromise);
            if (writeGoAway.isDone()) {
                processGoAwayWriteResult(channelHandlerContext, i4, j4, byteBuf, writeGoAway);
            } else {
                writeGoAway.addListener((GenericFutureListener<? extends Future<? super Void>>) new ChannelFutureListener() { // from class: io.netty.handler.codec.http2.Http2ConnectionHandler.3
                    @Override // io.netty.util.concurrent.GenericFutureListener
                    public void operationComplete(ChannelFuture channelFuture) throws Exception {
                        Http2ConnectionHandler.processGoAwayWriteResult(channelHandlerContext, i4, j4, byteBuf, channelFuture);
                    }
                });
            }
            return writeGoAway;
    ...
    protected void onConnectionError(ChannelHandlerContext channelHandlerContext, Throwable th, Http2Exception http2Exception) {
        ...
        if (AnonymousClass4.$SwitchMap$io$netty$handler$codec$http2$Http2Exception$ShutdownHint[http2Exception.shutdownHint().ordinal()] != 1) {
    ...
    @Override // io.netty.handler.codec.http2.Http2LifecycleManager
    public ChannelFuture resetStream(final ChannelHandlerContext channelHandlerContext, int i4, long j4, ChannelPromise channelPromise) {
        ChannelFuture writeRstStream;
        final Http2Stream stream = connection().stream(i4);
        if (stream != null && !stream.isResetSent()) {
            if (stream.state() == Http2Stream.State.IDLE) {
                writeRstStream = channelPromise.setSuccess();
            } else {
                writeRstStream = frameWriter().writeRstStream(channelHandlerContext, i4, j4, channelPromise);
            }
            stream.resetSent();
    ...
    public void gracefulShutdownTimeoutMillis(long j4) {
        if (j4 >= 0) {
            this.gracefulShutdownTimeoutMillis = j4;
            return;
        }
        throw new IllegalArgumentException("gracefulShutdownTimeoutMillis: " + j4 + " (expected: >= 0)");
    }
