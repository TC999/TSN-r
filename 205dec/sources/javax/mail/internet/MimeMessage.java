 ...
 */
        ...
        for (int i5 = 0; i5 < addressArr.length; i5++) {
            ...
            while (true) {
                if (i6 >= vector.size()) {
                    z3 = false;
                    break;
                } else if (((InternetAddress) vector.elementAt(i6)).equals(addressArr[i5])) {
                    i4++;
                    addressArr[i5] = null;
                    z3 = true;
                    break;
            ...
            if (!z3) {
                vector.addElement(addressArr[i5]);
            }
        }
        if (i4 != 0) {
            if (addressArr instanceof InternetAddress[]) {
                addressArr2 = new InternetAddress[addressArr.length - i4];
            } else {
                addressArr2 = new Address[addressArr.length - i4];
            }
            int i7 = 0;
            for (int i8 = 0; i8 < addressArr.length; i8++) {
                if (addressArr[i8] != null) {
                    addressArr2[i7] = addressArr[i8];
                    i7++;
                }
            }
            return addressArr2;
    ...
    @Override // javax.mail.Message
    public Message reply(boolean z3) throws MessagingException {
    ...
    public MimeMessage(MimeMessage mimeMessage) throws MessagingException {
        ...
        try {
            ...
            throw new MessagingException("IOException while copying message", e4);
        }
    }

    /* JADX INFO: Access modifiers changed from: protected */
    public MimeMessage(Folder folder, int i4) {
        super(folder, i4);
        this.modified = false;
        this.saved = false;
        this.strict = true;
    ...
    protected MimeMessage(Folder folder, InputStream inputStream, int i4) throws MessagingException {
        this(folder, i4);
        initStrict();
        parse(inputStream);
    }

    protected MimeMessage(Folder folder, InternetHeaders internetHeaders, byte[] bArr, int i4) throws MessagingException {
        this(folder, i4);
