...
public final class EpollDatagramChannel extends AbstractEpollChannel implements DatagramChannel {
    ...
    private static final String EXPECTED_TYPES = " (expected: " + StringUtil.simpleClassName((Class<?>) DatagramPacket.class) + ", " + StringUtil.simpleClassName((Class<?>) AddressedEnvelope.class) + Typography.f41568d + StringUtil.simpleClassName((Class<?>) ByteBuf.class) + ", " + StringUtil.simpleClassName((Class<?>) InetSocketAddress.class) + ">, " + StringUtil.simpleClassName((Class<?>) ByteBuf.class) + ')';
        ...
        public void connect(SocketAddress socketAddress, SocketAddress socketAddress2, ChannelPromise channelPromise) {
            ...
            try {
                try {
                    ...
                    epollDatagramChannel.local = epollDatagramChannel.mo12687fd().localAddress();
                } catch (Throwable th) {
                    th = th;
                }
                try {
        ...
        */
        public void epollInReady() {
            ...
            L35:
                ...
                io.netty.channel.unix.Socket r6 = r6.mo12687fd()     // Catch: java.lang.Throwable -> Lb4
            ...
            L5e:
                ...
                io.netty.channel.unix.Socket r7 = r7.mo12687fd()     // Catch: java.lang.Throwable -> Lb4
    ...
    private boolean doWriteMessage(Object obj) throws Exception {
        ...
        if (byteBuf.hasMemoryAddress()) {
            sendTo = mo12687fd().sendToAddress(byteBuf.memoryAddress(), byteBuf.readerIndex(), byteBuf.writerIndex(), inetSocketAddress.getAddress(), inetSocketAddress.getPort());
        } else if (byteBuf instanceof CompositeByteBuf) {
            IovArray cleanArray = ((EpollEventLoop) eventLoop()).cleanArray();
            cleanArray.add(byteBuf);
            sendTo = mo12687fd().sendToAddresses(cleanArray.memoryAddress(0), cleanArray.count(), inetSocketAddress.getAddress(), inetSocketAddress.getPort());
        } else {
            ByteBuffer internalNioBuffer = byteBuf.internalNioBuffer(byteBuf.readerIndex(), byteBuf.readableBytes());
            sendTo = mo12687fd().sendTo(internalNioBuffer, internalNioBuffer.position(), internalNioBuffer.limit(), inetSocketAddress.getAddress(), inetSocketAddress.getPort());
        }
        return sendTo > 0;
    }
        ...
        while (true) {
            ...
            try {
                ...
                if (Native.IS_SUPPORTING_SENDMMSG && channelOutboundBuffer.size() > 1 && (count = (nativeDatagramPacketArray = NativeDatagramPacketArray.getInstance(channelOutboundBuffer)).count()) >= 1) {
                    ...
                    int i = 0;
                    while (count > 0) {
                        int sendmmsg = Native.sendmmsg(mo12687fd().intValue(), packets, i, count);
                        if (sendmmsg == 0) {
    ...
    public boolean isActive() {
        return mo12687fd().isOpen() && ((((Boolean) this.config.getOption(ChannelOption.DATAGRAM_CHANNEL_ACTIVE_ON_REGISTRATION)).booleanValue() && isRegistered()) || this.active);
    }
