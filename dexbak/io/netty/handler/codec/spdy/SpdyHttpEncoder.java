...
public class SpdyHttpEncoder extends MessageToMessageEncoder<HttpObject> {
    ...
    private SpdyHeadersFrame createHeadersFrame(HttpResponse httpResponse) throws Exception {
        ...
        while (iteratorCharSequence.hasNext()) {
            ...
            defaultSpdySynReplyFrame.headers().add((SpdyHeaders) (this.headersToLowerCase ? AsciiString.m12682of(next.getKey()).toLowerCase() : (CharSequence) next.getKey()), (Object) next.getValue());
        }
    ...
    private SpdySynStreamFrame createSynStreamFrame(HttpRequest httpRequest) throws Exception {
        ...
        while (iteratorCharSequence.hasNext()) {
            Map.Entry<CharSequence, CharSequence> next = iteratorCharSequence.next();
            headers2.add((SpdyHeaders) (this.headersToLowerCase ? AsciiString.m12682of(next.getKey()).toLowerCase() : (CharSequence) next.getKey()), (Object) next.getValue());
        }
        ...
        if (!(httpObject instanceof HttpContent) || z) {
            ...
            if (httpContent instanceof LastHttpContent) {
                ...
                if (trailingHeaders.isEmpty()) {
                    ...
                    while (iteratorCharSequence.hasNext()) {
                        Map.Entry<CharSequence, CharSequence> next = iteratorCharSequence.next();
                        defaultSpdyHeadersFrame.headers().add((SpdyHeaders) (this.headersToLowerCase ? AsciiString.m12682of(next.getKey()).toLowerCase() : (CharSequence) next.getKey()), (Object) next.getValue());
                    }
                    list.add(defaultSpdyDataFrame);
