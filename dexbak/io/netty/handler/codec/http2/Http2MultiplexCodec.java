...
public final class Http2MultiplexCodec extends ChannelDuplexHandler {
    ...
    public final class Http2StreamChannel extends AbstractHttp2StreamChannel {
        ...
        protected void bytesConsumed(int i) {
            Http2MultiplexCodec.this.ctx.write(new DefaultHttp2WindowUpdateFrame(i).setStreamId(this.streamId));
        }
    ...
    public Http2MultiplexCodec(boolean z, EventLoopGroup eventLoopGroup, ChannelHandler channelHandler) {
    ...
    private ChannelFuture createStreamChannel(ChannelHandlerContext channelHandlerContext, int i, ChannelHandler channelHandler) {
        EventLoopGroup eventLoopGroup = this.streamGroup;
        if (eventLoopGroup == null) {
            eventLoopGroup = channelHandlerContext.channel().eventLoop();
        }
        Http2StreamChannel http2StreamChannel = new Http2StreamChannel(i);
    ...
    private boolean isLocalStream(int i) {
        return i > 0 && this.server == ((i & 1) == 0);
    }

    private void onStreamActive(int i) {
        this.childChannels.put(i, (int) ((Http2StreamChannel) createStreamChannel(this.ctx, i, this.streamHandler).channel()));
    }
    ...
    void writeFromStreamChannel(final Object obj, final boolean z) {
        final ChannelPromise newPromise = this.ctx.newPromise();
        EventExecutor executor = this.ctx.executor();
        if (executor.inEventLoop()) {
            writeFromStreamChannel0(obj, z, newPromise);
            return;
        }
        try {
            executor.execute(new Runnable() { // from class: io.netty.handler.codec.http2.Http2MultiplexCodec.3
                @Override // java.lang.Runnable
                public void run() {
                    Http2MultiplexCodec.this.writeFromStreamChannel0(obj, z, newPromise);
                }
            });
        } catch (Throwable th) {
