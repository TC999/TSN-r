...
public abstract class DnsNameResolverContext<T> {
    ...
    /* JADX INFO: Access modifiers changed from: package-private */
    /* renamed from: io.netty.resolver.dns.DnsNameResolverContext$4 */
    /* loaded from: E:\fuckcool\tsn\6400276.dex */
    public static /* synthetic */ class C139874 {
    ...
    private void finishResolve(Promise<T> promise) {
        ...
        this.resolveCache.cache(this.hostname, unknownHostException, this.parent.f40288ch.eventLoop());
    ...
    private void followCname(InetSocketAddress inetSocketAddress, String str, String str2, Promise<T> promise) {
        ...
        query(next, new DefaultDnsQuestion(str2, DnsRecordType.f40214A), promise);
    ...
    private boolean gotPreferredAddress() {
        ...
        int size = list.size();
        int i = C139874.$SwitchMap$io$netty$channel$socket$InternetProtocolFamily[this.resolveAddressTypes[0].ordinal()];
        if (i == 1) {
            for (int i2 = 0; i2 < size; i2++) {
                if (this.resolvedEntries.get(i2).address() instanceof Inet4Address) {
                    return true;
                }
            }
        } else if (i == 2) {
    ...
    public void internalResolve(Promise<T> promise) {
        ...
        InetSocketAddress next = this.nameServerAddrs.next();
        for (InternetProtocolFamily internetProtocolFamily : this.resolveAddressTypes) {
            int i = C139874.$SwitchMap$io$netty$channel$socket$InternetProtocolFamily[internetProtocolFamily.ordinal()];
            if (i == 1) {
                dnsRecordType = DnsRecordType.f40214A;
            } else if (i == 2) {
    ...
    private void onResponseAorAAAA(DnsRecordType dnsRecordType, DnsQuestion dnsQuestion, AddressedEnvelope<DnsResponse, InetSocketAddress> addressedEnvelope, Promise<T> promise) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (type == DnsRecordType.f40214A || type == DnsRecordType.AAAA) {
                ...
                if ((recordAt instanceof DnsRawRecord) && ((readableBytes = (content = ((ByteBufHolder) recordAt).content()).readableBytes()) == 4 || readableBytes == 16)) {
                    ...
                    try {
                        ...
                        this.resolveCache.cache(this.hostname, byAddress, recordAt.timeToLive(), this.parent.f40288ch.eventLoop());
