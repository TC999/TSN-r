...
public final class FlowableBuffer<T, C extends Collection<? super T>> extends AbstractFlowableWithUpstream<T, C> {
    ...
    static final class PublisherBufferExactSubscriber<T, C extends Collection<? super T>> implements FlowableSubscriber<T>, Subscription {
        ...
        public void onSubscribe(Subscription subscription) {
            if (SubscriptionHelper.validate(this.f40406s, subscription)) {
                this.f40406s = subscription;
        ...
        public void request(long j) {
            if (SubscriptionHelper.validate(j)) {
                this.f40406s.request(BackpressureHelper.multiplyCap(j, this.size));
            }
        }
    }
    ...
    static final class PublisherBufferOverlappingSubscriber<T, C extends Collection<? super T>> extends AtomicLong implements FlowableSubscriber<T>, Subscription, BooleanSupplier {
        ...
        @Override // io.reactivex.FlowableSubscriber, org.reactivestreams.Subscriber
        public void onSubscribe(Subscription subscription) {
            if (SubscriptionHelper.validate(this.f40407s, subscription)) {
                this.f40407s = subscription;
                this.actual.onSubscribe(this);
            }
        }
        ...
        public void request(long j) {
            ...
            if (!this.once.get() && this.once.compareAndSet(false, true)) {
                this.f40407s.request(BackpressureHelper.addCap(this.size, BackpressureHelper.multiplyCap(this.skip, j - 1)));
                return;
            }
            this.f40407s.request(BackpressureHelper.multiplyCap(this.skip, j));
        }
    }
    ...
    static final class PublisherBufferSkipSubscriber<T, C extends Collection<? super T>> extends AtomicInteger implements FlowableSubscriber<T>, Subscription {
        ...
        @Override // io.reactivex.FlowableSubscriber, org.reactivestreams.Subscriber
        public void onSubscribe(Subscription subscription) {
            if (SubscriptionHelper.validate(this.f40408s, subscription)) {
                this.f40408s = subscription;
                this.actual.onSubscribe(this);
            }
        }
        ...
        public void request(long j) {
            if (SubscriptionHelper.validate(j)) {
                if (get() == 0 && compareAndSet(0, 1)) {
                    this.f40408s.request(BackpressureHelper.addCap(BackpressureHelper.multiplyCap(j, this.size), BackpressureHelper.multiplyCap(this.skip - this.size, j - 1)));
                    return;
                }
                this.f40408s.request(BackpressureHelper.multiplyCap(this.skip, j));
            }
        }
    }
