    ...
    private boolean convertTo8Bit(MimePart mimePart) {
        ...
        try {
            if (mimePart.isMimeType("text/*")) {
                ...
                if (encoding != null && ((encoding.equalsIgnoreCase(HttpHeaders.Values.QUOTED_PRINTABLE) || encoding.equalsIgnoreCase(HttpHeaders.Values.BASE64)) && is8Bit(mimePart.getInputStream()))) {
                    mimePart.setContent(mimePart.getContent(), mimePart.getContentType());
                    mimePart.setHeader(HttpHeaders.Names.CONTENT_TRANSFER_ENCODING, "8bit");
                    z = true;
                }
        ...
        try {
            ...
            if (readServerResponse != 220) {
                ...
                if (this.debug) {
                    ...
                    printStream2.println("DEBUG SMTP: could not connect to host \"" + str + "\", port: " + port + ", response: " + readServerResponse + ShellAdbUtils.f33810d);
                }
                ...
                printStream3.println("DEBUG SMTP: connected to host \"" + str + "\", port: " + port + ShellAdbUtils.f33810d);
            }
    ...
    private void openServer() throws MessagingException {
        try {
            ...
            if (readServerResponse != 220) {
                ...
                if (this.debug) {
                    PrintStream printStream2 = this.out;
                    printStream2.println("DEBUG SMTP: got bad greeting from host \"" + hostName + "\", port: " + port + ", response: " + readServerResponse + ShellAdbUtils.f33810d);
                }
                throw new MessagingException("Got bad greeting from SMTP host: " + hostName + ", port: " + port + ", response: " + readServerResponse);
            } else if (this.debug) {
                PrintStream printStream3 = this.out;
                printStream3.println("DEBUG SMTP: protocol started to host \"" + hostName + "\", port: " + port + ShellAdbUtils.f33810d);
            }
        } catch (IOException e) {
