...
public class HorizontalBarHighlighter extends BarHighlighter {
    ...
    public List<Highlight> buildHighlights(IDataSet iDataSet, int i, float f, DataSet.Rounding rounding) {
        ...
        for (Entry entry : entriesForXValue) {
            ...
            arrayList.add(new Highlight(entry.getX(), entry.getY(), (float) pixelForValues.f23781x, (float) pixelForValues.f23782y, i, iDataSet.getAxisDependency()));
        }
        return arrayList;
    }
    ...
    public Highlight getHighlight(float f, float f2) {
        ...
        Highlight highlightForX = getHighlightForX((float) valsForTouch.f23782y, f2, f);
        if (highlightForX == null) {
            return null;
        }
        IBarDataSet iBarDataSet = (IBarDataSet) barData.getDataSetByIndex(highlightForX.getDataSetIndex());
        if (iBarDataSet.isStacked()) {
            return getStackedHighlight(highlightForX, iBarDataSet, (float) valsForTouch.f23782y, (float) valsForTouch.f23781x);
        }
